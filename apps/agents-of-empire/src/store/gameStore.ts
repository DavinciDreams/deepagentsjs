...
const MAX_AGENTS = 500; // Maximum number of agents allowed in the game
...
export const useGameStore = create<GameStore>()(
  immer((set, get) => ({
    ...
    cameraPosition: { x: 10, y: 30, z: 10 }, // Center on 20x20 world
    cameraTarget: { x: 10, y: 0, z: 10 },
    zoom: 1.5, // Start zoomed out a bit more to see more
  ...
  spawnAgent: (name, position, agentRef, parentId) => {
    ...
    if (get().agentCount >= MAX_AGENTS) {
      console.warn(`Cannot spawn agent "${name}": Maximum agent limit (${MAX_AGENTS}) reached`);
      return null as any;
    }
  ...
  spawnAgentBatch: (count, basePosition, pattern) => {
    ...
    // Calculate how many agents we can actually spawn
    const currentCount = get().agentCount;
    const remainingSlots = MAX_AGENTS - currentCount;
    const actualCount = Math.min(count, remainingSlots);

    if (actualCount < count) {
      console.warn(`Cannot spawn all ${count} agents: Only ${remainingSlots} slots remaining (max: ${MAX_AGENTS})`);
    }
    ...
    if (spawnPattern === "grid") {
      ...
      for (let i = 0; i < actualCount; i++) {
        ...
        const agent = get().spawnAgent(`Agent-${i + 1}`, position);
