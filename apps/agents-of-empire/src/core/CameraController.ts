...
export function useCameraController({
  ...
  // Get world size for bounds checking
      ...
      switch (e.key) {
        ...
        case "a":
          ...
          if (e.shiftKey) {
            ...
            setPosition({
              x: Math.max(0, Math.min(worldSize.width, position.x - panAmount)),
              y: position.y,
              z: position.z,
            });
          }
        ...
        case "w":
          setPosition({
            x: position.x,
            y: position.y,
            z: Math.max(0, Math.min(worldSize.height, position.z - panAmount)),
          });
          break;
        case "ArrowDown":
        case "s":
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isPanning) {
        ...
        const newX = Math.max(0, Math.min(worldSize.width, position.x - worldDeltaX));
        const newZ = Math.max(0, Math.min(worldSize.height, position.z - worldDeltaZ));
